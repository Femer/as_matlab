...
a Python wrapper for a fast solver generated by FORCES Pro
   ...
   OUTPUT = mpc_boatTack_h20_py.mpc_boatTack_h20_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = mpc_boatTack_h20_py.mpc_boatTack_h20_solve(PARAMS) returns additionally
   the integer EXITFLAG indicating the state of the solution with 
       1 - Optimal solution has been found (subject to desired accuracy)
       0 - Maximum number of interior point iterations reached
      -7 - Line search could not progress

   [OUTPUT, EXITFLAG, INFO] = mpc_boatTack_h20_py.mpc_boatTack_h20_solve(PARAMS) returns 
...
_lib = ctypes.CDLL(os.path.join(os.getcwd(),'mpc_boatTack_h20/lib/mpc_boatTack_h20.dll')) 
csolver = getattr(_lib,'mpc_boatTack_h20_solve')

class mpc_boatTack_h20_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
mpc_boatTack_h20_params = {'minusAExt_times_x0' : np.array([]),
'Hessians' : np.array([]),
...
class mpc_boatTack_h20_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
mpc_boatTack_h20_outputs = {'u0' : np.array([]),
}


class mpc_boatTack_h20_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(mpc_boatTack_h20_params_ctypes), ctypes.POINTER(mpc_boatTack_h20_outputs_ctypes), ctypes.POINTER(mpc_boatTack_h20_info) )
csolver.restype = ctypes.c_int

def mpc_boatTack_h20_solve(params):
	'''
a Python wrapper for a fast solver generated by FORCES Pro
 ...
 See also COPYING
	...
	# convert parameters
	params_py = mpc_boatTack_h20_params_ctypes()
	for par in params:
		try:
	...
	outputs_py = mpc_boatTack_h20_outputs_ctypes()
	info_py = mpc_boatTack_h20_info()
	try:
		exitflag = _lib.mpc_boatTack_h20_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py) )
	except:
		#print 'Problem with solver'
		raise

	# convert outputs
	for out in mpc_boatTack_h20_outputs:
		mpc_boatTack_h20_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return mpc_boatTack_h20_outputs,int(exitflag),info_py
